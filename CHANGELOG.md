...
## V2.4.23 (2026-08-10) — 试卷详情:每题直达编辑 + 一键改难度

- 试卷管理 → 点开某套卷的「查看」详情,每道题新增两个能力:
  1. **编辑按钮**:跳转到「题库管理」并自动打开该题的完整编辑弹窗(`/teacher?edit=<id>`,复用原有题干/选项/答案/解析/难度表单)。
  2. **一键改难度**:题干旁 1–5 星点击即改,直接调用 `PUT /questions/:id { difficulty }`,即时生效并刷新本地详情。
- 验证:`tsc --noEmit` 通过;部署后试卷详情出现「编辑」与星级控件。
