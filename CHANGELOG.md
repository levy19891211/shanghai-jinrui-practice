...
## V2.4.23 (2026-08-10) — 题库管理「按套题查看」:每题直达编辑 + 一键改难度

- 题库管理页新增「按套题查看」视图(工具栏切换按钮,默认仍为平铺列表):
  - 题目按 `paper` 自动分组,每组一个「查看 / 收起」按钮;
  - 展开后**每道题旁都有「编辑」按钮**,点击直接打开该题的完整编辑弹窗(题干/选项/答案/解析/难度),另附「审核 / 删除」快捷操作;
  - 每题题干旁附 **1–5 星一键改难度**,点击即调 `PUT /questions/:id { difficulty }` 即时生效并刷新列表。
- 撤销了此前误加到「试卷组卷」页面的编辑按钮/星级改动(非目标位置)。
- 验证:`tsc --noEmit` 通过;部署后题库管理工具栏出现「按套题查看」。
