...
## 阶段四:M1 前端(Vibe Coding 工具)

- [ ] `[VC]` 初始化 Next.js 项目(TypeScript + Tailwind + ESLint)+ `/api` 代理
- [ ] `[VC]` 登录 / 注册页面
- [ ] `[VC]` 刷题页(练习模式):题目展示、选项作答、即时对错与解析
- [ ] `[VC]` 题库管理页(老师):题目列表、新增/编辑/删除
- [ ] `[BOTH]` 联调:注册 → 刷题 → 提交判分 → 看成绩

## 阶段五:M2 考试模式

- [ ] `[VC]` 限时模拟考界面(倒计时、答题卡、防刷新丢进度)
- [ ] `[WB]` 模考交卷判分(超时自动提交)
- [ ] `[VC]` 成绩历史与趋势图

## 阶段六:M3 学情分析

- [ ] `[VC]` 错题本页面(按知识点分组、掌握标记)
- [ ] `[VC]` 知识点掌握度雷达图
- [ ] `[WB]` 老师学情统计 API(班级/个人)
- [ ] `[VC]` 老师学情报表页面
...
- [ ] `[BOTH]` 官方真题 PDF 结构化录入流程(下载 → 解析 → 校对 → 导入)
- [ ] `[WB]` 批量导入 API(Excel / CSV / JSON)
- [ ] `[WB]` 组卷功能(按知识点/难度/数量生成试卷)
- [ ] `[VC]` 导入与组卷界面
